...
const Header = () => {
  ...
  const handleSearch = (suggestion) => {
    dispatch(addText(suggestion));
  };

  return (
    <>
      <div className="grid grid-flow-col px-5 pt-5 shadow-sm fixed bg-white w-full z-40 ">
        ...
        <div className="ml-32 col-span-10">
          <div>
            <input
              className="outline-none border h-10 border-gray-400  rounded-l-full pr-64 pl-4 py-2"
              type="text"
            ...
            <button className=" bg-gray-200 px-4 py-2 rounded-r-full">
              Search
            </button>
          </div>
          <div>
                ...
                <ul>
                  {suggestions.map((suggestion) => (
                    <Link to="/results" key={suggestion}>
                      <li
                        className="px-2 mt-1 py-1 hover:bg-gray-50 hover:rounded-lg"
                        onClick={() => {
